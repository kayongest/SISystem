...
try {
    ...
    $query = "
        SELECT 
            ...
            e.title,
            e.source,
            COALESCE(NULLIF(e.date, '0000-00-00'), DATE(MIN(sm.created_at))) as date,
            COALESCE(NULLIF(NULLIF(e.location, '0'), ''), MAX(sm.destination_name)) as location,
            COALESCE(NULLIF(e.project_manager, ''), MAX(sm.project_manager)) as project_manager,
