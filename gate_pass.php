...
$stmt->close();
$stmt2->close();
// Removed manual connection close as it's a shared singleton
...
if (!$transport_required) {
    die("Access Denied: Gate Passes are exclusively restricted to transport-active movement types (Stock-to-Stock and Stock-to-Venue Transport).");
}
...
// Resolve correct driver name
$driver_display_name = $batch['transport_driver'] ?? 'N/A';
if ($driver_display_name === 'Valetine' || $driver_display_name === 'valentinb') {
    $driver_display_name = 'Bembeleza Valentin';
} else {
    // Check database to see if we can resolve the driver name to their full name
    $driver_conn = getConnection();
    $driver_stmt = $driver_conn->prepare("SELECT full_name FROM users WHERE role = 'driver' AND (username = ? OR full_name = ?)");
    if ($driver_stmt) {
        $driver_stmt->bind_param("ss", $driver_display_name, $driver_display_name);
        $driver_stmt->execute();
        $driver_res = $driver_stmt->get_result();
        if ($driver_row = $driver_res->fetch_assoc()) {
            $driver_display_name = $driver_row['full_name'];
        }
        $driver_stmt->close();
    }
    // Removed driver_conn->close() as it's a shared singleton
}

$pageTitle = "Equipment Gate Pass - " . $batch_id;
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="header-with-logo">
                            <div class="logo-section">
                             ...
                             <div class="title-section" style="flex-grow: 1;">
                                <div class="report-title">EQUIPMENT GATE PASS</div>
                        ...
                        <div class="info-section">
                            <table class="info-table-grid">
                                ...
                                <tr>
                                    ...
                                    <td class="info-value"><?php echo htmlspecialchars($driver_display_name); ?></td>
                                    <td class="info-label">VEHICLE</td>
                                    <td class="info-value"><?php echo htmlspecialchars($batch['transport_vehicle_type'] ?? 'N/A'); ?></td>
                                </tr>
                                <tr>
                                    <td class="info-label">ACCOMPANYING TECH</td>
                                    <td class="info-value" colspan="3"><?php echo htmlspecialchars($technician_name); ?></td>
                                </tr>
                            </table>
                        </div>
                    </td>
                    ...
                    <td>
                        ...
                        <div class="signature-section" style="grid-template-columns: repeat(3, 1fr);">
                            <div class="signature-card">
                                ...
                                <div class="sig-line">Stock Controller (Released By)</div>
                            </div>
                            <div class="signature-card">
                                <div class="digital-signature">
                                    <?php if (isset($batch['driver_verified']) && $batch['driver_verified']): ?>
                                        <span class="badge bg-success" style="font-size: 14px; padding: 10px 20px;"><i class="fas fa-check-circle"></i> VERIFIED</span>
                                    <?php endif; ?>
                                </div>
                                <div class="sig-name"><?php echo htmlspecialchars($driver_display_name); ?></div>
                                <div class="sig-line">Driver Verification</div>
                            </div>
                            <div class="signature-card">
                                ...
                                <div class="sig-line">Accompanying Tech (Verified)</div>
                            </div>
                        </div>
                    </td>
                </tr>
    ...
    <script>
        function downloadPDF() {
            ...
            const options = {
                ...
                filename: 'Equipment_Gate_Pass_<?php echo $batch_id; ?>.pdf',
