        ...
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('sw.js')
                    .then(reg => console.log('Service Worker registered for scan_bulk. Scope:', reg.scope))
    ...
    <div class="container-fluid px-2 pt-4">
        <div class="d-flex justify-content-between align-items-center mb-4 px-3">
            <h5 class="mb-0">
                ...
                <span id="connectionBadge" class="badge bg-success ms-2" style="font-size: 0.75rem;">
                    <i class="fas fa-signal me-1"></i> Online
                </span>
                <span id="offlineQueueBadge" class="badge bg-warning text-dark ms-2 d-none" style="font-size: 0.75rem;">
                    <i class="fas fa-cloud-upload-alt me-1"></i> <span id="offlineQueueCount">0</span> Pending Sync
                </span>
            </h5>
            <div class="btn-group">
        ...
        <script>
            ...
            function showConfirmationModal(title, message, onConfirm, onCancel) {
                ...
                const modalHtml = `
                <div class="modal fade animate__animated animate__fadeIn animate__faster" id="customConfirmModal" tabindex="-1" aria-labelledby="customConfirmModalLabel" aria-hidden="true" style="z-index: 1060;">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px);">
                            <div class="modal-header border-0 bg-light rounded-top" style="border-top-left-radius: 16px !important; border-top-right-radius: 16px !important; padding: 1.25rem 1.5rem;">
                                <h5 class="modal-title fw-bold text-dark d-flex align-items-center" id="customConfirmModalLabel">
                                    <i class="fas fa-question-circle text-primary me-2 fs-4"></i> ${title}
                                </h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body py-4 px-4">
                                <p class="text-dark mb-0 fs-6" style="line-height: 1.5; font-size: 1.05rem !important;">${message.replace(/\\n/g, '<br>').replace(/\n/g, '<br>')}</p>
                            </div>
                            <div class="modal-footer border-0 px-4 pb-4 pt-0 d-flex gap-2">
                                <button type="button" class="btn btn-light rounded-pill px-4 btn-sm fw-bold border" id="confirmModalCancelBtn" data-bs-dismiss="modal">Cancel</button>
                                <button type="button" class="btn btn-primary rounded-pill px-4 btn-sm fw-bold" id="confirmModalOkBtn">OK</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            function findItemInOfflineCatalog(searchTerm) {
                try {
                    const catalogRaw = localStorage.getItem('offline_catalog');
                    if (!catalogRaw) return null;
                    const catalog = JSON.parse(catalogRaw);
                    
                    // Search by ID first
                    const termNum = parseInt(searchTerm, 10);
                    if (!isNaN(termNum)) {
                        const item = catalog.find(i => i.id === termNum);
                        if (item) return item;
                    }
                    
                    // Search by Serial Number
                    let item = catalog.find(i => i.serial && i.serial.toLowerCase() === searchTerm.toLowerCase().trim());
                    if (item) return item;
                    
                    // Search by exact name or contains name
                    item = catalog.find(i => i.name && i.name.toLowerCase().includes(searchTerm.toLowerCase().trim()));
                    return item || null;
                } catch (e) {
                    console.error('Error reading offline catalog:', e);
                    return null;
                }
            }
            ...
            function fetchItemById(itemId, fallbackName, fallbackSerial) {
                ...
                // OFFLINE CHECK
                if (!navigator.onLine) {
                    console.log('Offline: Looking up item in local catalog cache');
                    const localItem = findItemInOfflineCatalog(itemId);
                    if (localItem) {
                        addToBatch({
                            id: localItem.id,
                            name: localItem.name,
                            serial_number: localItem.serial || 'N/A',
                            quantity: 1,
                            status: localItem.status || 'available',
                            stock_location: localItem.location || 'N/A'
                        });
                        showNotification('info', `Item resolved offline: ${localItem.name}`);
                    } else if (fallbackName) {
                        showConfirmationModal(
                            "Offline Add",
                            `[OFFLINE] Item ID ${itemId} not in offline cache.\n\nName: ${fallbackName}\nSerial: ${fallbackSerial || 'N/A'}\n\nAdd anyway?`,
                            function() {
                                addToBatch({
                                    id: itemId,
                                    name: fallbackName,
                                    ...
                                    quantity: 1,
                                    status: 'available',
                                    stock_location: 'Offline Add'
                                });
                            }
                        );
                    } else {
                        showNotification('warning', `[OFFLINE] Item ID ${itemId} not found in offline cache`);
                    }
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success && response.item) {
                            if (response.item.is_case && response.item.nested_items && response.item.nested_items.length > 0) {
                                const caseName = response.item.item_name;
                                const itemCount = response.item.nested_items.length;
                                showConfirmationModal(
                                    "Case Detected",
                                    `Scanned "${caseName}" which contains ${itemCount} packed items.\n\nDo you want to automatically add all ${itemCount} items to this batch?`,
                                    function() {
                                        // Add Case + Nested Items
                                        addToBatch({
                                            id: response.item.id,
                                        ...
                                        response.item.nested_items.forEach(nestedItem => {
                                            addToBatch({
                                                id: nestedItem.id,
                                                name: nestedItem.item_name,
                                                serial_number: nestedItem.serial_number,
                                                quantity: 1,
                                                status: nestedItem.status,
                                                stock_location: nestedItem.stock_location || 'N/A',
                                                image: nestedItem.image || null,
                                                totalStock: 1
                                            });
                                        });
                                        showNotification('success', `Added "${caseName}" and all ${itemCount} packed items.`);
                                    },
                                    function() {
                                        // Only Add Case itself
                                        addToBatch({
                                            id: response.item.id,
                                            name: response.item.item_name,
                                            serial_number: response.item.serial_number,
                                        ...
                                        showNotification('info', `Added case "${caseName}" only.`);
                                    }
                                );
                                return;
                            }
                            ...
                            showConfirmationModal(
                                "Add New Item",
                                `Item ID ${itemId} not found in database.\n\nName: ${fallbackName}\nSerial: ${fallbackSerial || 'N/A'}\n\nAdd as new item to batch?`,
                                function() {
                                    addToBatch({
                                        id: itemId,
            ...
            function fetchItemFromDatabase(searchTerm) {
                ...
                // OFFLINE CHECK
                if (!navigator.onLine) {
                    console.log('Offline: Searching offline catalog for:', searchTerm);
                    const localItem = findItemInOfflineCatalog(searchTerm);
                    if (localItem) {
                        addToBatch({
                            id: localItem.id,
                            name: localItem.name,
                        ...
                        showNotification('warning', `[OFFLINE] "${searchTerm}" not found in offline cache`);
                    }
                    return;
                }
            ...
            function clearBatch() {
                ...
                showConfirmationModal(
                    "Clear Batch",
                    `Are you sure you want to clear all ${batchItems.length} items from this batch?`,
                    function() {
                        batchItems = [];
                        updateBatchUI();
                        saveBatchToStorage();
        ...
        `;
            ...
            // ========== PWA & OFFLINE-FIRST FUNCTIONS ==========
            function queueOfflineBatch(batchData) {
                try {
                    const queue = JSON.parse(localStorage.getItem('offline_batches_queue') || '[]');
                    
                    // Add local temporary batch number for UI display
                    const localBatchNum = 'OFFLINE-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
                    batchData.batch_number = localBatchNum;
                    batchData.items_details = [...batchItems]; // cache item details for success modal display
                    
                    queue.push(batchData);
                    localStorage.setItem('offline_batches_queue', JSON.stringify(queue));
                    ...
                    showNotification('warning', 'Device is offline. Batch saved to Offline Sync Queue!');
                    
                    // Trigger success modal with offline message
                    setTimeout(() => {
                        const successModalElement = document.getElementById('submissionSuccessModal');
                        if (successModalElement) {
                            document.getElementById('successStockControllerName').textContent = batchData.stock_controller_name || 'Stock Controller';
                            document.getElementById('successItemCountBadge').textContent = `${batchData.items_details.length} Item(s) (Offline Queued)`;
                            document.getElementById('successBatchIdDisplay').textContent = localBatchNum;
                            
                            const cTimer = document.getElementById('approvalCountdown');
                            if (cTimer) cTimer.textContent = 'Offline (Pending Sync)';
                            
                            const infoEl = document.getElementById('countdownInstructionText');
                            if (infoEl) {
                                infoEl.innerHTML = `Your batch is queued offline. It will automatically submit once internet connection is restored.`;
                            }
                            
                            const sModal = new bootstrap.Modal(successModalElement);
                            sModal.show();
                        }
                    }, 500);
                } catch (e) {
                    console.error('Failed to queue offline batch:', e);
                    showNotification('error', 'Failed to save batch offline: ' + e.message);
                }
            }
            ...
            function syncOfflineCatalog() {
                ...
                console.log('Online: Syncing offline items catalog...');
                $.ajax({
                    url: 'api/get_items_offline_cache.php',
                    method: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.success && response.items) {
                            localStorage.setItem('offline_catalog', JSON.stringify(response.items));
                            localStorage.setItem('offline_catalog_timestamp', response.timestamp);
                            console.log(`Offline catalog synced: ${response.count} items cached.`);
                        }
                    },
                    error: function(err) {
                        console.warn('Failed to sync offline items catalog:', err);
                    }
                });
            }

            function syncOfflineBatches() {
                if (!navigator.onLine) return;
                
                const queue = JSON.parse(localStorage.getItem('offline_batches_queue') || '[]');
                if (queue.length === 0) return;
                
                console.log(`Online: Found ${queue.length} offline batches to synchronize.`);
                showNotification('info', `Syncing ${queue.length} offline batches...`);
                ...
                function processNext() {
                    if (queue.length === 0) {
                        localStorage.setItem('offline_batches_queue', '[]');
                        updateOfflineQueueUI();
                        if (syncCount > 0) {
                            showNotification('success', `Successfully synchronized ${syncCount} batch(es)!`);
                        }
                        return;
                    }
                    
                    const batch = queue.shift();
                    const localId = batch.batch_number;
                    // Remove local temporary fields before sending to server
                    delete batch.batch_number;
                    delete batch.items_details;
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                syncCount++;
                                console.log(`Offline batch sync success: ${localId} -> ${response.batch_number}`);
                            } else {
                                failCount++;
                                console.error(`Offline batch sync failed for ${localId}:`, response.message);
                            }
                            localStorage.setItem('offline_batches_queue', JSON.stringify(queue));
                            updateOfflineQueueUI();
                            processNext();
                        ...
                        error: function(xhr, status, err) {
                            failCount++;
                            console.error(`Network error syncing batch ${localId}:`, err);
                            // Stop syncing for now if network failed again
                            queue.unshift(batch);
                            localStorage.setItem('offline_batches_queue', JSON.stringify(queue));
                            updateOfflineQueueUI();
                        }
            ...
            function updateOfflineQueueUI() {
                try {
                    ...
                    if (count > 0) {
                        ...
                        if (offlineQueueBadge) offlineQueueBadge.classList.remove('d-none');
                    } else {
                        if (offlineQueueBadge) offlineQueueBadge.classList.add('d-none');
                    }
                } catch (e) {
                    console.error('Error updating offline queue UI:', e);
                }
            }
            ...
            function updateConnectionStatus() {
                ...
                if (badge) {
                    if (isOnline) {
                        ...
                        badge.innerHTML = '<i class="fas fa-signal me-1"></i> Online';
                        syncOfflineCatalog();
                        syncOfflineBatches();
                    } else {
                        badge.className = 'badge bg-warning text-dark ms-2';
                        badge.innerHTML = '<i class="fas fa-plane-slash me-1"></i> Offline Mode';
                    }
                }
                updateOfflineQueueUI();
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                // Initialize Offline Checks & Listeners
